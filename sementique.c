...
void store_const(){
	while((tabTOKEN[l->smbl.CODE]!=ALGO_TOKEN)&&(tabTOKEN[l->smbl.CODE]!=VAR_TOKEN)&&(tabTOKEN[l->smbl.CODE]!=DEBUT_TOKEN)){
		if(tabTOKEN[l->smbl.CODE]==ID_TOKEN){
			strcpy((TAB_IDFS+i)->NOM,l->smbl.NOM);
...
void store_var(){
	while((tabTOKEN[l->smbl.CODE]!=CONST_TOKEN)&&(tabTOKEN[l->smbl.CODE]!=ALGORITHME_TOKEN)&&(tabTOKEN[l->smbl.CODE]!=DEBUT_TOKEN)){
		if(tabTOKEN[l->smbl.CODE]==ID_TOKEN){
             regle2();
			strcpy((TAB_IDFS+i)->NOM,l->smbl.NOM);
...
void regle1(){ //chercher  identificateur titi non déclaré
     smbl=smbl_sem;
...
void regle2(){ // chercher si il a une double déclaration
	int r=0;
	for (int i = 0; i <NbrIDFS ; ++i)
	{
		if(strcmp(head->data->nom,(TAB_IDFS+i)->NOM)==0){
...
void regle3(){
    smbl=smbl_sem;
	while(tabTOKEN[l->smbl.CODE]!=DEBUT_TOKEN) Symbole_Suiv();
